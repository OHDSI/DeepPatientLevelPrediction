...
TemporalDataset <- torch::dataset(
  ...
  initialize = function(data, labels = NULL, numericalIndex = NULL) {
    ...
    nonTemporalNumericalCovariateIds <- index[which(nonTemporalNumericalIndex)]
    nonTemporalCategoricalCovariateIds <- index[which(!nonTemporalNumericalIndex)]
    ...
    maxTemporalFeaturesPerRow <- temporalData %>% dplyr::group_by(rowId) %>% 
      dplyr::summarise(n=n()) %>% dplyr::summarise(max=max(n)) %>% dplyr::pull()
