...
Hyperparameter tuning is controlled by PatientLevelPrediction through
`PatientLevelPrediction::createHyperparameterSettings()`. Grid search evaluates
every possible combination of model and estimator hyperparameters, random
search samples from those combinations, and custom search can supply candidate
sets or an adaptive generator. Since neural networks can be very flexible and
have many hyperparameter combinations, it is almost never feasible to do a full
...
combinations are 2*2*2*2 or 16 but specify `sampleSize = 10` to only try 10 of
those.
...
To tune against a different metric, pass a tuning metric to the same
hyperparameter settings object:

```{r, eval=FALSE}
auprcMetric <- PatientLevelPrediction::createTuningMetric(
  fun = PatientLevelPrediction::computeAuprc,
  maximize = TRUE,
  name = "AUPRC"
)
...
Use the `hyperparameterSettings` argument to `PatientLevelPrediction::runPlp()`.
Create it with `PatientLevelPrediction::createHyperparameterSettings()` and set
`search` to `"grid"`, `"random"`, or `"custom"`.
