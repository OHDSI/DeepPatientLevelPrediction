...
FROM (
  ...
  FLOOR(DATEDIFF(@time_part, @cdm_database_schema.@domain_table.@domain_start_date, cohort.cohort_start_date)*1.0/@time_interval ) as time_id,
    ...
    CASE WHEN @cdm_database_schema.@domain_table.@domain_start_date <= cohort.cohort_start_date 
    THEN
     DATEDIFF(@time_part, @cdm_database_schema.@domain_table.@domain_start_date, @cdm_database_schema.@domain_table.@domain_start_date)*1.0/@time_interval
     ELSE 
