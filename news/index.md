...
including paper-style numeric embeddings, schedules, and data-dependent
initialization (PR
[\#171](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/171)) -
Add dropout support to `TemporalPE` and test coverage for
positional-encoding dropout behavior (PR
[\#165](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/165))

Bug fixes - Replace deprecated `pynvml` with `nvidia-ml-py` (PR
[\#167](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/167),
closes
[\#166](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/166)) -
Improve expand-component handling and add regression tests (PR
[\#164](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/164)) -
Fix estimator seed casting, prediction shaping, and custom-embedding /
Poincare model paths for current torch and reticulate behavior (PR
[\#171](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/171))

Runtime and compatibility - Relax the torch requirement to
`torch>=2.7,<3` and validate against `torch==2.10.0` in stable CI lanes
(PR
[\#171](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/171)) -
Add a Python 3.10 compatibility lane using the latest available torch
(PR
[\#171](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/171)) -
Clean `torchinductor_runner` artifacts during `R CMD check` (PR
[\#171](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/171))

Documentation and CI - Update installation guidance to recommend `uv`
and Python 3.12 (PR
[\#171](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/171)) -
Update GitHub Actions checks to use Python 3.12 by default and fix
Windows torch installation handling (PR
[\#171](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/171)) -
Update pkgdown link checking for current `lychee` CLI behavior (PR
[\#171](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/171))

## DeepPatientLevelPrediction 2.2.0

- \[Feature\] Add positional encodings that use `timeId` (PR
  [\#154](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/154)):
  ...
  - `TemporalPE`: Combines an absolute temporal embedding and a semantic
    one
  - `StochasticConvPE`: Relative learnable positional embedding
    (`convSPE`)
...
- \[Feature\] Temporal data processing added (PR
  [\#147](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/147))
- \[CI\] Use uv for python in github actions (PR
  [\#136](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/136))
- \[Feature\] Add an option to use torch compile (PR
  [\#133](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/133))
- \[Feature\] More efficient conversions from polars to torch in dataset
  processing (PR
  [\#133](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/133))
- \[CI\] Automatically detect broken links in docs using github actions
- \[Feature\] Initialize models with embeddings, including Poincare (PR
  [\#125](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/125)
  and PR
  [\#130](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/130))
- \[Feature\] Model initialization made more flexible with classes (PR
  [\#124](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/124))

## DeepPatientLevelPrediction 2.1.0

- Added basic transfer learning functionality. See
  vignette(“TransferLearning”)
- Add a gpu memory cleaner to clean cached memory after out of memory
  error
- The python module torch is now accessed through an exported function
  instead of loading the module at package load
- Added gradient accumulation. Studies running at different sites using
  different hardware can now use same effective batch size by
  accumulating gradients.
- Refactored out the cross validation from the hyperparameter tuning
- Remove predictions from non-optimal hyperparameter combinations to
  save space
...
- Fixed a bug when resuming from a full TrainingCache (thanks Zoey Jiang
  and Linying Zhang )
- Updated installation documentation after feedback from HADES hackathon
- Fixed a bug where order of numeric features wasn’t conserved between
  training and test set
- TrainingCache now only saves prediction dataframe for the best
  performing model
...
- Fix actions after torch updated to v0.10
  ([\#65](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/65))

## DeepPatientLevelPrediction 1.1.2

- Fix bug introduced by removing modelType from attributes
  ([\#59](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/59))

## DeepPatientLevelPrediction 1.1

- Check for if number of heads is compatible with embedding dimension
  fixed
  ([\#55](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/55))
- Now transformer width can be specified as a ratio of the embedding
  dimensions (dimToken),
  ([\#53](https://github.com/OHDSI/DeepPatientLevelPrediction/issues/53))
- A custom metric can now be defined for earlyStopping and learning rate
  schedule
...
- used vectorised torch operations to speed up data conversion in torch
  dataset
